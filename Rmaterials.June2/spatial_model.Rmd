...
This uses the terra package - terra requires various programs to be implemented on your computer

C++11, GDAL (>= 2.2.3), GEOS (>= 3.4.0), PROJ (>= 4.9.3)

There are website that can help you install these
...
Here's an example of reading in data from a "tif" file - a commonly used data type for spatial data

We can also use the terra package to do some spatial analysis -of terrain data (digital elevation model - DEM) to get things like slope and aspect

```{r simple, echo=FALSE}

# read in from a tif file
dem = rast("tifs/filleddem.tif")
names(dem)
# change the name from filleddem to dem
names(dem) = c("dem")

# plot
plot(dem)
# you can create summary statistics as always
summary(dem)

# spatial data (as imported from  the tif)
crs(dem)
...
now do some modeling - lets build a model of risk based on precipitation data, a classification of slope and soil erodibility

For precipitation - we will also spatially interpolation precipitation from a meterological station

For slope we will do the classification

For soil erodibility, we will assume its spatially constant (but could use a map in the future)
...
# we can use app to run a function for spatial data layers
site$precip = site$dem %>% app(fun=scale_pcp, base_elev=base_elev, precip_base=precip_base)
...
# try estimating risk with our precipitation map
