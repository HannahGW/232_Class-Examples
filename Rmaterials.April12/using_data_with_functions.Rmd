...
```{r getfunctions}
source("../Rmaterials.April12/power_gen.R")
power_gen
```
...
reservoir_model_res = reservoir_model_res %>% 
  mutate(Power=power_gen(height,flow))

# Always graph results to make sure they make sense
#View(reservoir_model_res)
ggplot(reservoir_model_res, aes(flow, Power)) +
  geom_point() +
  labs(x="Flow Rate m3/s", y="Power (W/s)")
...
Keff = seq(from=0.1, to=0.5, by=0.1)
length(Keff)
...
resg=as.data.frame(res) %>% 
  pivot_longer(everything(),names_to="Keff", values_to="Power")
...
reservoir_model_res$scen= seq(from=1, to=nrow(reservoir_model_res)) #scen assigns a unique id
...
resg=as.data.frame(res) %>% 
  pivot_longer(!scen,names_to="Keff", values_to="Power") # the !scen  makes it so it's not moved to multiple columns like Keff, it just stays in one. 
...
ggplot(all_res, aes(flow, Power, col=Keff)) +
  geom_point() +
  labs(x="Flow Rates m3/s", y="Power W/m2")
